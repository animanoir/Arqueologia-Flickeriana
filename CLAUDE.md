...
- **Territory data**: [src/territories.js](src/territories.js) exports `countries` — Mexico (the original hand-curated list, first = default), France, Germany, Japan, UK, US. Each territory maps a label to a comma-separated Flickr tag list following the curation model: territory name + local-language/native-script variants (kanji for Japan, umlauts for Germany) + historic/former names and abbreviations (distritofederal/df/cdmx for Ciudad de México) + widely used exonyms (tokio, londres) + most-photographed cities and landmarks. Flickr's `photos.search` accepts at most 20 tags per query, so keep every list under that; ambiguous tags that mostly surface other places or subjects (león, toledo, naruto) are deliberately excluded. Each country's first territory is its default; the rest are alphabetical.
